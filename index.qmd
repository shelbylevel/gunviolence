...
# Firearm Deaths (Suicide, Assault, and Accident) by County, 2018-2022
...
```{r oh2022-map-gradient, fig.height=8, fig.width = 8}
...
# Firearm Deaths (Suicide, Assault, and Accident) by County Type, 2018-2022
...
oh1822 %>%
  ...
  geom_col(
    alpha=0.75, 
    fill = c('#573D62', '#7A5689', '#B79DC2', '#D4C4DA')
    ) +  
  labs(caption = "", 
...
# Firearm Deaths (Suicide, Assault, and Accident) by County Type, 2022
...
read.delim(here::here(
  'data',
  'oh-metro-top3-agg-yr-2018-22.txt'
  )) %>% 
  mutate(
    Rate = (Deaths/Population)*100000
    ) %>%
...
read.delim(here::here(
  'data',
  'oh-suburb-top3-agg-yr-2018-22.txt'
  )) %>% 
  mutate(
    Rate = (Deaths/Population)*100000
    ) %>%
...
read.delim(here::here(
  'data',
  'oh-ruralapp-top3-agg-yr-2018-22.txt'
  )) %>% 
  mutate(
    Rate = (Deaths/Population)*100000
    ) %>%
  ...
  mutate(
    Notes = 'Rural Appalachian'
    ) -> ruralapp

read.delim(here::here(
  'data',
  'oh-ruralnonapp-top3-agg-yr-2018-22.txt'
  )) %>% 
  mutate(
    Rate = (Deaths/Population)*100000
    ) %>%
  ...
  mutate(
    Notes = 'Rural Non-Appalachian'
    ) -> ruralnonapp

rbind(metro, suburb, ruralapp, ruralnonapp) -> oh_types_agg

```

```{r cty-types-agg2-bar, include=TRUE}
...
oh_types_agg$Notes <- factor(oh_types_agg$Notes) %>% 
  fct_reorder(-oh_types_agg$Rate)
...
oh_types_agg %>%
  ...
  geom_col(
    alpha=0.75, 
    fill = c('#7A5689', '#B79DC2', '#573D62', '#D4C4DA')
    ) +  
  labs(caption = "", 
       y="Rate per 100,000 population",
       x="") +
...
# Suicide by Firearm Deaths by County Type, 2022
...
rbind(metro, suburb, ruralapp, ruralnonapp) -> oh_types

```

```{r suicide-cty-types-bar, include=TRUE}
...
types_suicide$Notes <- factor(types_suicide$Notes) %>% 
  fct_reorder(-types_suicide$Rate)
...
# Assault by Firearm Deaths by County Type, 2022

```{r assault-cty-types-bar, include=TRUE}
...
types_assault$Notes <- factor(types_assault$Notes) %>% 
  fct_reorder(-types_assault$Rate)
